...
function postPatrolPath(coordinates: [number, number][]) {
  fetch(`${API_BASE_URL}/patrol-path/`, {
    ...
    if (!response.ok) {
      ...
      console.error('Patrol path rejected:', body.error ?? response.statusText);
    }
  });
}
...
export function usePatrolPathDrawing(map: L.Map | null) {
  ...
  const finalize = () => {
    ...
    if (points.length >= MIN_VERTICES) {
      postPatrolPath(points.map((p) => [p.lat, p.lng]));
    }
    clearPreview();
